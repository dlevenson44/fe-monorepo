...
const TextInput: FunctionComponent<TextInputProps> = ({
  ...
  const responsiveLabelStyles = useMemo(
    () =>
      shrink
        ? 'translate-y-3 scale-75 text-black bg-white w-1/4'
  ...
  return (
    <div className={`relative flex flex-col w-1/2 ${containerStyle}`}>
      ...
      />
      <label
        className={`absolute pointer-events-none origin-top-left transition-transform left-3 bottom-7 ${responsiveLabelStyles} ${labelStyle}`}
      >
        {label}
      </label>
    </div>
